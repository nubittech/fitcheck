  ...
  return (
    <div className="signup-page">
      ...
      <div className="signup-hero">
        ...
        <p>Share outfits, get feedback, no influencers. Just real style.</p>
      </div>
      ...
      <form className="signup-form" onSubmit={handleSubmit}>
        <div className="signup-field-wrap">
          <label className={`signup-field ${errors.fullName ? 'error' : ''}`}>
            ...
            <input type="text" placeholder="Full Name" value={fullName} onChange={(e) => handleFieldChange('fullName', e.target.value, setFullName)} />
          </label>
        ...
        <div className="signup-field-wrap">
          <label className={`signup-field ${errors.email ? 'error' : ''}`}>
            ...
            <input type="email" placeholder="Email Address" value={email} onChange={(e) => handleFieldChange('email', e.target.value, setEmail)} />
          </label>
        ...
        <div className="signup-field-wrap">
          <label className={`signup-field ${errors.password ? 'error' : ''}`}>
            ...
            <input type={showPassword ? 'text' : 'password'} placeholder="Password" value={password} onChange={(e) => handleFieldChange('password', e.target.value, setPassword)} />
        ...
        <div className="signup-field-wrap">
          <label className={`signup-field ${errors.confirmPassword ? 'error' : ''}`}>
            ...
            <input type="password" placeholder="Confirm Password" value={confirmPassword} onChange={(e) => handleFieldChange('confirmPassword', e.target.value, setConfirmPassword)} />
          </label>
          {errors.confirmPassword && <p className="signup-error">{errors.confirmPassword}</p>}
        </div>

        {apiError && <p className="signup-api-error" style={{ color: apiError.includes('Check your email') ? '#10B981' : '#EF4444', fontSize: '13px', textAlign: 'center', marginBottom: '8px' }}>{apiError}</p>}
      ...
      <p className="signup-login-row">
        Already a member? <button type="button" onClick={onGoLogin}>Log in</button>
      </p>
    </div>
  )
}
