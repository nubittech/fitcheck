-- Migration: Add purchased boosts support
-- Run this in Supabase SQL Editor

-- 1. Add purchased_boosts_balance column to profiles
ALTER TABLE profiles
ADD COLUMN IF NOT EXISTS purchased_boosts_balance INT DEFAULT 0;

-- 2. Function to credit a purchased boost (called after RevenueCat purchase succeeds)
...
BEGIN
  UPDATE profiles
  SET purchased_boosts_balance = COALESCE(purchased_boosts_balance, 0) + 1
...
-- 3. Update activate_boost to support purchased boosts and new limits (free=0, premium=3)
CREATE OR REPLACE FUNCTION activate_boost(user_id_param UUID)
RETURNS JSON AS $$
DECLARE
  user_record RECORD;
  max_boosts INT;
  period_ms BIGINT := 30 * 24 * 60 * 60 * 1000;
  now_ms BIGINT;
  reset_ms BIGINT;
BEGIN
  ...
  IF user_record.is_premium THEN
    max_boosts := 3;
    reset_ms := (EXTRACT(EPOCH FROM COALESCE(user_record.boosts_reset_at, NOW() - INTERVAL '31 days')) * 1000)::BIGINT;

    -- Reset monthly boosts if period has passed
    IF (now_ms - reset_ms) > period_ms THEN
      UPDATE profiles
